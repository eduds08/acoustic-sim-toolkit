...
class WebGPUConfig(SimulationConfig):
    def __init__(self, **simulation_config):
        ...
        # Workgroup Sizes (Goes into @workgroup_size in .wgsl files)
        for i in range(15, 0, -1):
            if (self.grid_size_z % i) == 0:
                self.wsz = i  # workgroup z size
                break

        for i in range(15, 0, -1):
            if (self.grid_size_x % i) == 0:
                self.wsx = i  # workgroup x size
                break
